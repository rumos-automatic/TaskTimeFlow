...
## [0.9.12] - 2025-07-04 ⏰ **タスク時間編集の柔軟性向上**

### 🐛 修正
- **⏰ タスク推定時間の編集問題を修正**
  - タイムラインからタスク作成時に時間が編集できない問題を解決
  - 次のタスクまでの時間を自動計算しつつ、手動編集も可能に

### 🔧 改善
- **📝 時間入力UI の改善**
  - ドロップダウン形式から数値入力フィールドに変更
  - 5分〜480分（8時間）の範囲で自由に時間設定可能
  - リアルタイムでの入力検証機能
  - フォーカスアウト時の自動補正機能（最小5分、最大480分）

### 🔧 技術的改善
- タスクフォームコンポーネントの入力制御を改善
- タイムラインでの時間計算ロジックを柔軟に対応
- 事前定義された時間オプションの廃止
