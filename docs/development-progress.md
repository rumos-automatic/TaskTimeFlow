...
### モバイルタイムライン操作性の大幅改善
- ✅ **モバイル専用操作モード**
  - 長押し（500ms）で操作モード選択UIを表示
  - 移動モード：タスクカードのドラッグ移動に特化
  - リサイズモード：上下エッジで時間調整に特化
  - ハプティックフィードバック（振動）で操作開始を通知
- ✅ **操作競合の完全解決**
  - タイムラインスクロールとエッジ操作の競合を解決
  - 各モードで必要な操作のみを有効化
  - 誤操作を防ぐモード外操作の無効化
- ✅ **視覚的フィードバック強化**
  - 操作モードインジケーターで現在状態を常時表示
  - モード中はカードに青いリング表示
  - 即座にモード解除可能なキャンセルボタン
  - リサイズモードでのみエッジハンドルを表示
- ✅ **技術的実装**
  - タッチイベントの最適化（10px移動閾値）
  - モード別条件付きイベントリスナー
  - ドラッグ終了時の自動モードリセット
  - useSortableのdisabled属性による制御

### 操作性の更なる改善（v0.9.19.1）
- ✅ **操作モードの簡略化**
  - 長押し後のダイアログを削除し、即座に操作モードを有効化
  - 移動とリサイズを同時に可能にする統合操作モード
  - より直感的でスピーディーな操作体験を実現
- ✅ **自動スクロール機能の実装**
  - ドラッグ中に画面端に近づくと自動的にスクロール
  - requestAnimationFrameによる滑らかなスクロール制御
  - エッジ検出による段階的なスクロール速度調整（80px閾値）
  - スクロール可能な親要素を自動検出
- ✅ **スクロール競合の根本解決**
  - カード移動とタイムラインスクロールが協調動作
  - 操作中のみドラッグ＆ドロップを有効化
  - より自然な操作フローを実現

### バグ修正（v0.9.19.2）
- ✅ **自動スクロールの逆方向問題を修正**
  - リサイズ時に画面が逆方向にスクロールする問題を解決
  - 毎フレームで距離を再計算するように改善
  - マウス/タッチ位置の追跡を最適化
- ✅ **移動モードのドラッグ機能を修正**
  - @dnd-kitのlistenersとattributesの適用条件を修正
  - PC版とモバイル版で適切な条件分岐を実装
  - タッチイベントとの競合を解決
